...
describe('/#/privacy-security/change-password', () => {
  ...
  describe('as Morty', () => {
    protractor.beforeEach.login({ email: 'morty@' + config.get('application.domain'), password: require('../helpers/passwords').morty() })
  ...
  describe('challenge "changePasswordBenderChallenge"', () => {
    protractor.beforeEach.login({ email: 'bender@' + config.get('application.domain'), password: require('../helpers/passwords').bender() })
